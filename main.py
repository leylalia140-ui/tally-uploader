...
# One-off recovery for the 60 videos orphaned on 08.08.2026 (Margaret/Yuki batches
# interrupted by deploys wiping in-memory PENDING_APPROVALS before Bjarne could act).
# Niche per file reconstructed from that day's deploy logs — each batch's "Parsed:"
# line was matched against the exact file list it processed, see chat history.
_RECOVERY_NICHE_MAP = {
...
_RECOVERY_DATE_FOLDER = "8th August 2026"


@app.post("/admin/recover_unresolved")
async def admin_recover_unresolved(x_admin_secret: Optional[str] = Header(None)):
    if not settings.ADMIN_SECRET or x_admin_secret != settings.ADMIN_SECRET:
        raise HTTPException(status_code=403, detail="forbidden")
    ...
    if not unresolved:
        return {"recovered": [], "not_found_in_drive": [], "no_niche_mapping": []}

    drive = GoogleDriveClient()
    recovered, not_found, no_mapping = [], [], []
    by_model_content: dict[tuple[str, str], list[dict]] = {}

    for old_token, entry in unresolved:
        model_name = entry["model_name"]
        file_name = entry["file_name"]
        niche = _RECOVERY_NICHE_MAP.get((model_name, file_name))
        if niche is None:
            no_mapping.append(f"{model_name} — {file_name}")
            continue
        try:
            folder_id = drive.resolve_folder_path(
                ["Models", model_name, "Full AI Content", "edited", _RECOVERY_DATE_FOLDER, "Videos"]
            )
            file_id = drive.find_file(file_name, folder_id)
            if not file_id:
                not_found.append(f"{model_name} — {file_name}")
                continue
            tmp_path = os.path.join(tempfile.gettempdir(), f"recover_{uuid.uuid4().hex}_{file_name}")
            drive.download_file(file_id, tmp_path)
        except Exception as e:
            logger.error(f"Recovery download failed for {model_name}/{file_name}: {e}")
    ...
    return {"recovered": recovered, "not_found_in_drive": not_found, "no_niche_mapping": no_mapping}
